...
export default function Breadcrumb({ path, onRewind }: BreadcrumbProps) {
  return (
    <div className="w-full overflow-x-auto pb-1 hide-scrollbar">
      <div className="flex items-center gap-1.5 whitespace-nowrap min-w-max text-xs">
        {path.map((step, index) => {
          ...
          const canRewind = isFilm && !isLast && !!onRewind;

          return (
            <div key={`${step.id}-${index}`} className="flex items-center gap-1.5">
              <div
                onClick={canRewind ? () => onRewind(index) : undefined}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-[4px] border font-medium transition-all ${
                  isLast
                    ...
                    : canRewind
                    ? 'bg-bg-secondary border-border text-text-secondary hover:text-white hover:border-text-secondary cursor-pointer'
                    : 'bg-transparent border-transparent text-text-muted cursor-default'
                }`}
                title={canRewind ? `Rewind to ${step.name}` : step.name}
              >
